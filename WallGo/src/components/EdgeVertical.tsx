...
export const EdgeVertical: React.FC<EdgeProps> = ({
  row,
  col,
  currentPlayer,
  onClaim,
  ...
  const canPlaceWall = () => {
    ...
    const wallExists = allWalls.some(
      (wall) => wall.type === "v" && wall.row === row && wall.col === col
    );
    ...
    // For vertical edges, check if it's the left or right edge of the piece
    // Left edge: col = pieceCol, row = pieceRow
    // Right edge: col = pieceCol + 1, row = pieceRow
    return (
      (col === pieceCol && row === pieceRow) ||
      (col === pieceCol + 1 && row === pieceRow)
    );
  };
